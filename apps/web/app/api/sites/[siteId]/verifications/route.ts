...
export async function POST(request: Request, context: { params: Promise<{ siteId: string }> }) {
  ...
  try {
    ...
    if (provider !== "google" && provider !== "bing") {
      throw new IgleError("INVALID_PROVIDER", "Provider must be google or bing.", 400);
    }

    await runtime.verificationService.add(site, provider, code, (await requireActor()));

    if (wantsRedirect) {
      return NextResponse.redirect(new URL(`/sites/${site.id}/scripts?verificationAdded=1`, resolveRequestOrigin(request)), { status: 303 });
    }
