  ...
  try {
    ...
    if (wantsRedirect) {
      return NextResponse.redirect(
        new URL(`/sites/${site.id}?restored=${revision.revisionNumber}`, resolveRequestOrigin(request)),
        { status: 303 }
      );
    }
    ...
    if (wantsRedirect) {
      return NextResponse.redirect(
        new URL(`/sites/${siteId}?restoreError=${encodeURIComponent(formatted.body.error.message)}`, resolveRequestOrigin(request)),
        { status: 303 }
      );
    }
