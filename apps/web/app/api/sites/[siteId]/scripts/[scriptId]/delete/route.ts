...
export async function POST(request: Request, context: { params: Promise<{ siteId: string; scriptId: string }> }) {
  ...
  try {
    ...
    await runtime.scriptService.deleteScript(site, scriptId, (await requireActor()));

    if (wantsRedirect) {
      return NextResponse.redirect(new URL(`/sites/${site.id}/scripts?deleted=1`, resolveRequestOrigin(request)), { status: 303 });
    }
