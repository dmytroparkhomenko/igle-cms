...
export async function POST(request: Request, context: { params: Promise<{ siteId: string }> }) {
  ...
  try {
    ...
    const result = await runtime.mirrorService.resync(site, (await requireActor()));

    if (wantsRedirect) {
      return NextResponse.redirect(new URL(`/sites/${site.id}?mirrorSynced=${result.copiedPages}`, resolveRequestOrigin(request)), { status: 303 });
    }
