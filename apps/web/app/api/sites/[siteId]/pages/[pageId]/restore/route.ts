...
export async function POST(request: Request, context: { params: Promise<{ siteId: string; pageId: string }> }) {
  ...
  try {
    ...
    await runtime.pageService.restore(site, pageId, (await requireActor()));

    if (wantsRedirect) {
      return NextResponse.redirect(new URL(`/sites/${site.id}/trash?pageRestored=1`, request.url), { status: 303 });
    }
    ...
    if (wantsRedirect) {
      return NextResponse.redirect(
        new URL(`/sites/${siteId}/trash?pageError=${encodeURIComponent(formatted.body.error.message)}`, request.url),
        { status: 303 }
      );
    }
