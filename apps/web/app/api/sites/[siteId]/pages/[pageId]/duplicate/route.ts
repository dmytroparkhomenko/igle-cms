...
export async function POST(request: Request, context: { params: Promise<{ siteId: string; pageId: string }> }) {
  ...
  try {
    ...
    const result = await runtime.pageService.duplicate(site, pageId, (await requireActor()));

    if (wantsRedirect) {
      return NextResponse.redirect(
        new URL(`/sites/${site.id}/pages/${result.page.id}?duplicated=1`, resolveRequestOrigin(request)),
        { status: 303 }
      );
    }
