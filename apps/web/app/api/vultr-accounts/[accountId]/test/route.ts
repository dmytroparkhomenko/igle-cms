...
export async function POST(request: Request, context: { params: Promise<{ accountId: string }> }) {
  ...
  try {
    ...
    const result = await runtime.vultrAccountService.testConnection(accountId, actor);

    if (wantsRedirect) {
      const url = result.ok
        ? `/servers?vultrTestOk=${result.instanceCount ?? 0}`
        : `/servers?vultrTestError=${encodeURIComponent(result.error ?? "Connection failed.")}`;
