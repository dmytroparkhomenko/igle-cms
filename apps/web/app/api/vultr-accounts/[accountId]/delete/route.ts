...
export async function POST(request: Request, context: { params: Promise<{ accountId: string }> }) {
  ...
  try {
    ...
    await runtime.vultrAccountService.remove(accountId, actor);

    if (wantsRedirect) {
      return NextResponse.redirect(new URL("/servers?vultrRemoved=1", resolveRequestOrigin(request)), { status: 303 });
    }
