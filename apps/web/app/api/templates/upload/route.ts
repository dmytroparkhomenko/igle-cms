...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (wantsRedirect) {
      return NextResponse.redirect(new URL(`/templates/${key}?uploaded=1`, request.url), { status: 303 });
    }
    ...
    if (wantsRedirect) {
      return NextResponse.redirect(
        new URL(`/templates/upload?error=${encodeURIComponent(formatted.body.error.message)}`, request.url),
        { status: 303 }
      );
    }
