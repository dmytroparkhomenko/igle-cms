...
export async function POST(request: Request, context: { params: Promise<{ serverId: string }> }) {
  ...
  try {
    ...
    await runtime.serverService.requestImport(serverId, actor);

    if (wantsRedirect) {
      return NextResponse.redirect(new URL("/servers?importQueued=1", resolveRequestOrigin(request)), { status: 303 });
    }
