...
export async function POST(request: Request, context: { params: Promise<{ domainId: string }> }) {
  ...
  try {
    ...
    const result = await runtime.domainService.issueOriginSsl(domainId, actor);

    if (wantsRedirect) {
      return NextResponse.redirect(new URL("/domains", resolveRequestOrigin(request)), { status: 303 });
    }
