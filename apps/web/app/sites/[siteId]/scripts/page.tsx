...
export default async function ScriptsPage({
  ...
  const site = await runtime.siteService.get(
    siteId,
    ...
    <>
      <div className="toolbar">
        <div>
          ...
          <p className="muted">
            Custom snippets injected at build time by placement, environment and
            page.
          </p>
        </div>
        <Link
      ...
      <p className="muted" style={{ marginTop: -8, fontSize: 13.5 }}>
        ...
        Igle builds the right filename and content. Don&apos;t forget to deploy the site to update the verification code.
      </p>
      <div className="list" style={{ marginBottom: 16 }}>
        {verifications.map((verification) => (
          ...
          >
            ...
            <form
              method="post"
              action={`/api/sites/${site.id}/verifications/${verification.id}/check`}
            >
              <button
                className="button"
                type="submit"
      ...
      >
        <form
          className="card"
          method="post"
          action={`/api/sites/${site.id}/verifications`}
          style={{ display: "grid", gap: 10 }}
        >
          <h2 style={{ margin: 0, fontSize: 15 }}>Google Search Console</h2>
          <input type="hidden" name="provider" value="google" />
          <p className="muted" style={{ margin: 0, fontSize: 12.5 }}>
            Search Console → Settings → Ownership verification → HTML file
            method. Google gives you a file named something like{" "}
            <code>google2a96ccfcce2f2467.html</code> — paste just the middle
            part, without the leading <code>google</code> and without the
            trailing <code>.html</code> (Igle adds both back when it builds the
            file). Pasting the whole filename also works — it&apos;s stripped
            automatically — but the bare code is clearer.
          </p>
          ...
          <input
            ...
            id="googleCode"
            name="code"
            placeholder="e.g. 2a96ccfcce2f2467"
            required
          />
          <button
            className="button"
        ...
        >
          ...
          <p className="muted" style={{ margin: 0, fontSize: 12.5 }}>
            Webmaster Tools → Settings → Verify ownership → XML file method.
            Paste just the code inside <code>&lt;user&gt;</code>.
          </p>
      ...
      <div className="list" style={{ marginBottom: 28 }}>
        {scripts.map((script) => (
          ...
          >
            ...
            <form
              method="post"
              action={`/api/sites/${site.id}/scripts/${script.id}/toggle`}
            >
              <input
                type="hidden"
                name="enabled"
                value={script.enabled ? "false" : "true"}
              />
              <button
                className="button"
                type="submit"
        ...
        {scripts.length === 0 ? (
          <div className="list-row">
            <div className="main">
              ...
              <p className="muted">
                Add Google Analytics, GTM, or a custom snippet below.
              </p>
            </div>
          </div>
        ) : null}
      ...
      >
        <form
          className="card"
          method="post"
          action={`/api/sites/${site.id}/scripts/preset`}
          style={{ display: "grid", gap: 10 }}
        >
      ...
      >
        ...
        >
          <div>
            ...
            <select id="placement" name="placement" defaultValue="head-end">
