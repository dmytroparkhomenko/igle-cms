...
export default async function SiteDetailPage({
  ...
  const trashedCount = state.pages.filter((page) => page.siteId === site.id && page.deletedAt).length;
  ...
  const previewUrl = `${previewOrigin}/${site.slug}/`;
  ...
  const assignedServer = assignedServerId ? selectableServers.find((server) => server.id === assignedServerId) : undefined;
  const aapanelDomain = site.metadata.deploymentTarget === "aapanel" ? site.metadata.domain : undefined;
  const [aapanelDns, aapanelSiteLookup] =
    assignedServerId && aapanelDomain
      ? await Promise.all([
          runtime.deployService.checkAaPanelDns(assignedServerId, actor, aapanelDomain).catch(() => null),
          runtime.deployService
            .findExistingAaPanelSite(assignedServerId, actor, aapanelDomain)
            .then((site_) => ({ ok: true as const, site: site_ }))
            .catch((error: unknown) => ({ ok: false as const, error: error instanceof Error ? error.message : "Lookup failed." }))
        ])
      : [null, null];
  const aapanelDomainOptions = assignedServerId
    ? await runtime.deployService.listAaPanelSites(assignedServerId, actor).catch(() => [])
    : [];
  ...
  const sitemapBase = site.metadata.domain ? `${protocol}://${site.metadata.domain}` : "https://your-domain.example";
  ...
  const revisionsWithDiff = await Promise.all(
    shownRevisions.map(async (revision) => {
      ...
      try {
        const diff = await runtime.revisionService.compare(site, revision.revisionNumber - 1, revision.revisionNumber);
    ...
    <>
      ...
      <div className="toolbar">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <a className="button" href={previewUrl} target="_blank" rel="noreferrer" style={{ background: "none", color: "var(--accent)" }}>
            Open preview
          </a>
      ...
      <div className="version-tiles">
        <div className="version-tile">
          ...
          <p className="version-tile-meta">
            {revisions[0] ? `${revisions[0].title} · ${new Date(revisions[0].createdAt).toLocaleString()}` : "No revisions yet"}
          </p>
        </div>
        <div className="version-tile">
          ...
          {productionRevision ? (
            <>
              <p className="version-tile-value" style={{ color: "var(--accent)" }}>
                #{productionRevision.revisionNumber}
              </p>
              <p className="version-tile-meta">
                {revisionsAhead > 0 ? `Draft ${revisionsAhead} revision${revisionsAhead === 1 ? "" : "s"} ahead` : "Matches last saved version"}
              </p>
            </>
          ) : (
            <>
              <p className="version-tile-value muted">Never deployed</p>
              <p className="version-tile-meta">Click &quot;Deploy to production&quot; above to publish the last saved version.</p>
            </>
          )}
        </div>
      </div>
      ...
      {pageDeleted ? (
        <article className="card" style={{ borderColor: "var(--accent)", marginBottom: 16 }}>
          Page moved to trash. <Link href={`/sites/${site.id}/trash`}>View trash</Link>
        </article>
      ) : null}
      {pageError ? (
        <article className="card" style={{ borderColor: "var(--warn)", marginBottom: 16 }}>
      ...
      <div className="card settings-card">
        <div className="settings-card-header">
          ...
          <div className="quick-links">
            <Link href={`/deployments?siteId=${site.id}`}>Deployment history</Link>
        ...
        <form method="post" action={`/api/sites/${site.id}/settings`}>
          <div className="settings-section">
            ...
            <div className="field">
              ...
              <input type="text" id="name" name="name" defaultValue={site.metadata.name} required />
            </div>
            <div className="field">
              <label htmlFor="domain">Domain</label>
              <DomainPicker initialValue={site.metadata.domain ?? ""} options={aapanelDomainOptions} />
            </div>
          </div>
          ...
          <div className="settings-section">
            ...
            <div className="field-row">
              <div className="field">
                ...
                <input type="text" id="language" name="language" defaultValue={site.metadata.language} maxLength={2} placeholder="en" />
              </div>
              <div className="field">
                <label htmlFor="country">Country (GEO)</label>
                <input type="text" id="country" name="country" defaultValue={site.metadata.country ?? ""} maxLength={2} placeholder="US" />
              </div>
            </div>
            <p className="muted" style={{ margin: 0, fontSize: 11.5 }}>
              Sets <code>&lt;html lang&gt;</code> for new pages and the site&apos;s GEO for SEO defaults. Existing pages keep
              their current <code>lang</code> attribute — this doesn&apos;t rewrite them.
            </p>
          </div>
          ...
          <div className="settings-section">
            ...
            <label className="muted" style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13 }}>
              <input type="checkbox" name="https" defaultChecked={site.metadata.https} />
              Force HTTPS
            </label>
            {selectableServers.length > 0 ? (
              <div className="field">
                <label htmlFor="deploymentTarget">Deploy to</label>
                <select id="deploymentTarget" name="deploymentTarget" defaultValue={site.metadata.deploymentTarget}>
            ...
            {site.metadata.deploymentTarget === "aapanel" ? (
              <div className="field">
                ...
                <select id="serverId" name="serverId" defaultValue={assignedServerId ?? ""}>
                ...
                {!assignedServerId ? (
                  <p className="muted" style={{ margin: "4px 0 0", fontSize: 11.5 }}>
                    Pick which registered server this site deploys to. Changing this later moves the site — its next
                    deploy uploads to the new server; the old copy is left in place unless you remove it there yourself.
                  </p>
                ) : null}
              </div>
            ) : null}
            {site.metadata.deploymentTarget === "aapanel" && assignedServer ? (
              <p className="muted" style={{ margin: 0, fontSize: 11.5 }}>
                Deploys create/reuse a site on <strong>{assignedServer.name}</strong> for{" "}
                <code>{site.metadata.domain || "the domain above"}</code>, upload the build there, and request SSL if
                &quot;Force HTTPS&quot; is on. Set the domain above before deploying.
              </p>
            ) : null}
            {aapanelDomain && aapanelDns ? (
              <p style={{ margin: 0, fontSize: 11.5, color: aapanelDns.matches ? "var(--accent)" : "var(--warn)" }}>
                {aapanelDns.matches
            ...
            {aapanelDomain && aapanelSiteLookup ? (
              <p className="muted" style={{ margin: 0, fontSize: 11.5, color: aapanelSiteLookup.ok ? undefined : "var(--warn)" }}>
      ...
      <details className="card" style={{ marginBottom: 28, maxWidth: 640 }}>
        <summary style={{ cursor: "pointer", fontWeight: 600 }}>Sitemap &amp; Robots</summary>
        <form method="post" action={`/api/sites/${site.id}/sitemap-robots`} className="mode-switch" style={{ display: "grid", gap: 10, marginTop: 16 }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <input type="checkbox" name="sitemapEnabled" defaultChecked={site.metadata.sitemap.enabled} />
            Generate sitemap.xml at build
          </label>
          {excludedFromSitemap.length > 0 ? (
            <p className="muted" style={{ margin: 0, fontSize: 12.5 }}>
              {excludedFromSitemap.length} page{excludedFromSitemap.length === 1 ? "" : "s"} excluded (set per-page under
              Additional SEO Settings): {excludedFromSitemap.map((page) => page.route).join(", ")}
            </p>
          ) : null}

          <p className="muted" style={{ margin: "10px 0 0", fontSize: 12.5 }}>robots.txt</p>

          <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <input type="radio" name="robotsMode" value="cms-generated" defaultChecked={site.metadata.robots.mode !== "manual"} />
            CMS-generated — written automatically from these settings
          </label>
          <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <input type="radio" name="robotsMode" value="manual" defaultChecked={site.metadata.robots.mode === "manual"} />
          ...
          <div className="mode-panel" data-panel="cms-generated">
            <p className="muted" style={{ margin: "0 0 6px", fontSize: 12.5 }}>
              This is what gets written at build/deploy time. It updates automatically from the settings above — there&apos;s
              nothing to edit here. Switch to Manual if you need rules this can&apos;t express (custom disallow rules,
              crawl-delay, multiple bot blocks).
            </p>
          ...
          <div className="mode-panel" data-panel="manual">
            <p className="muted" style={{ margin: "0 0 6px", fontSize: 12.5 }}>
              Written exactly as entered — Igle never modifies this file once it&apos;s in Manual mode.
            </p>
            <textarea
              name="robotsContent"
              defaultValue={site.metadata.robots.mode === "manual" ? site.metadata.robots.content ?? currentRobotsTxt : currentRobotsTxt}
      ...
      <div className="toolbar" style={{ marginBottom: 8 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <Link href={`/sites/${site.id}/trash`} className="button" style={{ background: "none", color: "var(--muted)" }}>
            Trash{trashedCount > 0 ? ` (${trashedCount})` : ""}
          </Link>
          <Link href={`/sites/${site.id}/bulk-seo`} className="button" style={{ background: "none", color: "var(--accent)" }}>
            Bulk edit SEO
          </Link>
        </div>
      </div>
      ...
      <div className="list" style={{ marginBottom: 28 }}>
        {revisionsWithDiff.map(({ revision, diff }) => (
          <div className="list-row" key={revision.id} style={{ alignItems: "flex-start" }}>
              ...
              <summary style={{ cursor: "pointer" }}>
                <span style={{ display: "inline-block" }}>
                  ...
                  <p className="muted" style={{ margin: "2px 0 0", fontSize: 12.5 }}>
                    ...
                    {revision.id === site.productionRevisionId ? " · Production" : ""}
                    {revision.revisionNumber === headRevisionNumber ? " · Current" : ""}
                  </p>
                </span>
              </summary>
              <div style={{ marginTop: 10 }}>
                {diff ? (
                  ...
                  <p className="muted" style={{ fontSize: 12.5 }}>
                    {revision.revisionNumber <= 1 ? "Initial revision — nothing to compare against." : "No file changes recorded."}
                  </p>
                )}
              </div>
            </details>
            {revision.revisionNumber !== headRevisionNumber ? (
              <form method="post" action={`/api/sites/${site.id}/revisions/${revision.revisionNumber}/restore`}>
      ...
      {revisions.length > shownRevisions.length ? (
        <p style={{ marginTop: -18, marginBottom: 28 }}>
          <Link href={`/sites/${site.id}?revisionsLimit=${revisionsLimit + 10}#revisions`} className="button" style={{ background: "none", color: "var(--accent)" }}>
            Load more revisions
          </Link>
        </p>
      ) : null}

      <details className="card danger-zone" style={{ marginBottom: 28, maxWidth: 480 }}>
        <summary style={{ cursor: "pointer", fontWeight: 600, color: "var(--warn)" }}>Danger Zone</summary>
        {deleteError ? <p style={{ color: "var(--warn)", fontSize: 12.5 }}>{deleteError}</p> : null}
        <form method="post" action={`/api/sites/${site.id}/delete`} style={{ display: "grid", gap: 8, marginTop: 12 }}>
          <p className="muted" style={{ margin: 0, fontSize: 12.5 }}>
            Permanently deletes this site&apos;s files and full revision history. This cannot be undone. Type the site slug (
            <strong>{site.slug}</strong>) to confirm.
          </p>
          <input type="text" name="confirmSlug" placeholder={site.slug} autoComplete="off" />
          <button className="button" type="submit" style={{ background: "var(--warn)", borderColor: "var(--warn)", justifySelf: "start" }}>
