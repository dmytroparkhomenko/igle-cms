...
export class ServerService {
  ...
  async add(input: { name: string; baseUrl: string; apiKey: string; restricted: boolean }, actor: Actor): Promise<ServerRecord> {
    assertCan(actor, "servers.manage");
    ...
    const server: ServerRecord = { id: id("server"), name, baseUrl, apiKey, restricted: input.restricted, createdAt: new Date().toISOString() };
  ...
  async update(serverId: string, input: { name?: string; baseUrl?: string; apiKey?: string; restricted?: boolean }, actor: Actor): Promise<void> {
    assertCan(actor, "servers.manage");
    await this.stateStore.update((state) => {
